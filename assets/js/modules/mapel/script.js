$(document).ready(function () {
  let submit = () => {
    $('#mapel_form').submit(function (e) {
      e.preventDefault();
      $('#loading').addClass('hidden');
      $('#mapel_load').removeClass('hidden');
      var form = $('#mapel_form')[0]; // You need to use standard javascript object here
    ...
    $.ajax({
      ...
      success: function (re) {
        ...
        if (re.status == 'success') {
          var elem = document.getElementById("mapel_pro");
          var width = 1;
          var id = setInterval(frame, 40);
          $('#mapel_success_load').removeClass('hidden');
          upload(re.data);
  ...
  let upload = (postfile) => {
    $.ajax({
      ...
      url: _URL + "mapel/insert",
      success: function (result) {
        if (result.status) {
          var elem = document.getElementById("mapel_success_pro");
          var width = 1;
